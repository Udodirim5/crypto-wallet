    ...
    >
      ...
      <div className="sticky top-0 flex justify-between items-center py-4 px-6 bg-[#0c0f0e] z-10">
        ...
        <div className="flex gap-3 sm:gap-4">
          <BsBellSlashFill className="hidden sm:block h-5 w-5 text-gray-400 hover:text-white transition-colors" />
          <PiWarningCircleFill className="hidden sm:block h-5 w-5 text-gray-400 hover:text-white transition-colors" />
        </div>
      </div>
      {/* COIN CONTENT */}
      <div className="flex flex-col gap-8 justify-center items-center mt-8 px-2">
        <div className="flex flex-col items-center mb-4">
          <div className="relative">
            <img
              src={token.logo}
              alt={token.name}
              className="absolute h-3 w-3 sm:h-4 sm:w-4 rounded-full bottom-3 right-2"
            />
            {token.networkLogo && (
              <img
        ...
        {/* ACTION BUTTONS */}
        <div className="flex gap-2 sm:gap-3 w-full max-w-md justify-center flex-wrap">
          ...
          <>
            ...
            <div className="flex flex-col gap-2">
              {Object.entries(groupedTransactions).map(
                ([label, transactions]) => (
                  <div key={label}>
                    <h3 className="text-sm text-gray-400 font-medium mb-2 px-4">
                      {label}
                    </h3>
